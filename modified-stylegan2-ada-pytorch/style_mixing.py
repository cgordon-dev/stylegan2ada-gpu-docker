...
def generate_style_mix(
    ...
    # Compute additional inference metrics
    num_images = len(row_seeds) * len(col_seeds)
    ...
    # Save timing and inference metrics into CSV
    log_file = os.path.join(outdir, "timing_log.csv")
    file_exists = os.path.isfile(log_file)
    with open(log_file, mode='a', newline='') as f:
        writer = csv.writer(f)
        if not file_exists:
            # If the CSV does not exist yet, create it with header
            writer.writerow([
                'network',
                'row_seeds',
                'col_seeds',
